...
function renderDashboard(overrides: Partial<React.ComponentProps<typeof ManagerDashboard>> = {}) {
  return render(<ManagerDashboard lifecycle={{ kind: 'not-installed' }} resource={null} resourceCompatible={false} resourceAmbiguous={false} resourceContradiction={false} activeAction={null} error={null} onInstall={vi.fn()} onTeardown={vi.fn()} onRetry={vi.fn()} {...overrides} />);
}
describe('ManagerDashboard', () => {
  ...
  it('renders a compatible installed resource and requires approval for every connection', () => { renderDashboard({ lifecycle: { kind: 'installed', runId: 'run-2', operationBusy: false }, resource, resourceCompatible: true }); expect(screen.getByRole('heading', { name: 'PostgreSQL is installed' })).toBeVisible(); expect(screen.getByText('Required for every connection request')).toBeVisible(); expect(screen.queryByRole('button', { name: /reset remembered/i })).not.toBeInTheDocument(); });
  ...
  it('prioritizes contradiction over a persisted progress lifecycle', () => { renderDashboard({ lifecycle: { kind: 'installing', runId: 'run-1' }, resource, resourceCompatible: true, resourceContradiction: true }); expect(screen.getByRole('heading', { name: 'PostgreSQL state needs recovery' })).toBeVisible(); expect(screen.queryByText('Installing PostgreSQL')).not.toBeInTheDocument(); });
});
