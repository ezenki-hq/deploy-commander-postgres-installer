...
describe('ManagerDashboard', () => {
  it('renders run-backed installation progress after reload', () => {
  ...
  it('renders a compatible installed resource and requires approval for every connection', () => {
    renderDashboard({
  ...
  it('shows busy connection runs while preserving installed identity', () => {
    renderDashboard({
      lifecycle: { kind: 'installed', runId: 'connection-1', operationBusy: true },
      resource,
      resourceCompatible: true,
    });
    expect(screen.getByRole('heading', { name: 'PostgreSQL is installed' })).toBeVisible();
    expect(screen.getByText(/operation is already in progress/i)).toBeVisible();
    expect(screen.getByRole('button', { name: 'Teardown PostgreSQL' })).toBeDisabled();
  });
  it('offers teardown for an incompatible resource', () => {
    renderDashboard({
      lifecycle: { kind: 'installed', runId: 'run-1', operationBusy: false },
      resource,
      resourceCompatible: false,
    });
    expect(
      screen.getByRole('heading', { name: 'PostgreSQL resource needs recovery' }),
    ).toBeVisible();
    fireEvent.click(screen.getByRole('button', { name: 'Teardown PostgreSQL' }));
    expect(screen.getByRole('dialog')).toBeVisible();
  });
  it('prioritizes multiple resources', () => {
    renderDashboard({
      lifecycle: { kind: 'installed', runId: 'run-1', operationBusy: false },
      resource,
      resourceCompatible: true,
    ...
    expect(
      screen.getByRole('heading', { name: 'PostgreSQL resource state is ambiguous' }),
    ).toBeVisible();
  });
  it('renders a contradiction warning', () => {
    renderDashboard({
      lifecycle: { kind: 'not-installed' },
      resource,
      resourceCompatible: true,
      resourceContradiction: true,
    });
    expect(screen.getByRole('heading', { name: 'PostgreSQL state needs recovery' })).toBeVisible();
  });
  it('prioritizes contradiction over a persisted progress lifecycle', () => {
    renderDashboard({
      lifecycle: { kind: 'installing', runId: 'run-1' },
      resource,
      resourceCompatible: true,
      resourceContradiction: true,
    });
    expect(screen.getByRole('heading', { name: 'PostgreSQL state needs recovery' })).toBeVisible();
    expect(screen.queryByText('Installing PostgreSQL')).not.toBeInTheDocument();
  });
});
