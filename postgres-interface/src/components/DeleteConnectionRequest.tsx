...
export default function DeleteConnectionRequest({
  ...
  const [approval, setApproval] = useState<DeleteConnectionApprovalContext | null>(null);
  ...
  const progress = (
    <ManagerShell badge={{ label: 'Deleting', tone: 'progress' }}>
      <StatusPanel
        tone="progress"
        eyebrow="Logical database request"
        title={busy ? 'Deleting PostgreSQL connection' : 'Preparing PostgreSQL connection'}
        role="status"
      >
        The manager is validating ownership and removing the connection safely.
      </StatusPanel>
    </ManagerShell>
  );
  if (approval)
