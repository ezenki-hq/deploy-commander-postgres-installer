...
export async function installPostgres(deps: LifecycleDeps): Promise<void> {
  ...
  if (projection.kind === "installed" || projection.kind === "conflict") {
    throw new PostgresRequestError(409, projection.kind === "conflict" ? "Multiple PostgreSQL resources exist" : "PostgreSQL is already installed");
  }
...
export async function teardownPostgres(deps: LifecycleDeps): Promise<void> {
  ...
  const confirmed = await deps.requestConfirmation("Teardown will remove PostgreSQL and all databases. Continue?");
  if (!confirmed) return;
