...
import { createPostgresConnection, type CreateApprovalDecision, type CreateConnectionDeps } from "./createConnection";

const resource: RPC.ResourceItem = {
  id: "resource-1", manager: "postgres-manager", agent: "agent-1", type: "postgres", name: "postgres",
  external: false, created_at: "2026-09-20T00:00:00Z", updated_at: "2026-09-20T00:00:00Z",
};
...
const request = {
  ...
  metadata: { action: "create-connection" as const, requestedAccess: { scope: "database" as const, operation: "create" as const, database: "orders" }, labels: { team: "payments" } },
};
const summary = (id: string, labels: Record<string, string> = {}): RPC.ConnectionItem => ({
  id, manager: "consumer-1", resource: "resource-1", external: false,
  created_at: "2026-09-20T00:00:00Z", updated_at: "2026-09-20T00:00:00Z", labels,
});
const connection = (id: string, access: AccessRequest = request.metadata.requestedAccess, labels = summary(id).labels): PostgresConnection => ({
  item: summary(id, Object.keys(labels ?? {}).length ? labels ?? {} : {
    team: "payments", "postgres.access": "database", "postgres.database": "orders", "postgres.database-origin": "managed",
  }), managerId: "consumer-1", resourceId: "resource-1",
  authority: access.scope === "database"
    ? { access: "database", database: access.database, origin: "managed" }
    : { access: "full" },
  access, username: `dc_user_${"b".repeat(32)}`, password: "connection-secret",
  platformConnection: installation.platformConnection,
  metadata: { access, username: `dc_user_${"b".repeat(32)}`, password: "connection-secret", platform_connection: installation.platformConnection },
});
const details = (item: PostgresConnection): RPC.GetConnection => ({
  connection: item.item,
  config: { id: item.item.id, manager: item.managerId, resource: item.resourceId, metadata: item.metadata },
});

function createDeps(overrides: Partial<CreateConnectionDeps> = {}) {
  const approval = vi.fn(async (): Promise<CreateApprovalDecision> => ({ allowed: true, access: request.metadata.requestedAccess }));
  const caller = fakeCaller({
    getMyResources: vi.fn().mockResolvedValue({ items: [resource], limit: 50, offset: 0, total: 1 }),
    getResource: vi.fn().mockResolvedValue({ resource, config: {
      id: resource.id, manager: resource.manager, agent: "agent-1", resource_type: "postgres", name: "postgres",
  ...
  const deps: CreateConnectionDeps = {
    ...
    runTracker: { startAndWait: vi.fn().mockResolvedValue({ run: { status: 2, finished_at: "now" }, config: {} }), dispose: vi.fn() },
    generateLoginCredentials: vi.fn(async (): Promise<LoginCredentials> => ({ username: `dc_user_${"c".repeat(32)}`, password: "new-secret" })),
...
describe("createPostgresConnection", () => {
  ...
  it("requires approval before returning an exact existing connection", async () => {
    ...
    caller.getConnections = vi.fn().mockResolvedValue({ items: [existing.item], limit: 50, offset: 0, total: 1 });
  ...
  it("returns concrete errors for missing and conflicting resources", async () => {
    ...
    missing.caller.getMyResources = vi.fn().mockResolvedValue({ items: [], limit: 50, offset: 0, total: 0 });
    await expect(createPostgresConnection(missing.deps, request)).rejects.toMatchObject({ status: 404 });
    const conflict = createDeps();
    conflict.caller.getMyResources = vi.fn().mockResolvedValue({ items: [resource, { ...resource, id: "resource-2" }], limit: 50, offset: 0, total: 2 });
    await expect(createPostgresConnection(conflict.deps, request)).rejects.toMatchObject({ status: 409 });
  });

  it("passes labels-only requests through the approval decision", async () => {
    const labelsOnly = { ...request, metadata: { action: "create-connection" as const, requestedAccess: null, labels: { team: "payments" } } };
    const { deps, approval } = createDeps({ requestApproval: vi.fn(async (context) => {
  ...
  it("rejects an incompatible existing identity after approval", async () => {
    const existing = connection("connection-1", { scope: "database", operation: "existing", database: "orders" });
  ...
  it("starts a managed provision and returns the exact created connection", async () => {
    const created = connection("connection-created", request.metadata.requestedAccess, { team: "payments", "postgres.access": "database", "postgres.database": "orders", "postgres.database-origin": "managed" });
    const { deps, caller } = createDeps();
    ...
    expect(deps.runTracker.startAndWait).toHaveBeenCalledWith(expect.objectContaining({
      action: "create-connection", runner: "ezenki/deploy-commander-runner:latest",
  ...
  it.each([
    ["database-not-found", 404], ["database-collision", 409], [null, 500],
  ] as const)("maps run marker %s to status %s", async (marker, status) => {
    const { deps } = createDeps({ runTracker: { startAndWait: vi.fn().mockRejectedValue(new RunFailedError("run-1", marker)), dispose: vi.fn() } });
    await expect(createPostgresConnection(deps, request)).rejects.toMatchObject({ status });
    try { await createPostgresConnection(deps, request); } catch (error) {
