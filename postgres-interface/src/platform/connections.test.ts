...
const details = (connection: RPC.ConnectionItem, access = { scope: "database", operation: "create", database: "orders" }) => ({
  connection,
  config: {
    id: connection.id,
...
describe("listResourceConnections", () => {
  it("loads every connection page before calculating database peers", async () => {
    ...
    const caller = fakeCaller({
      ...
      getConnection: vi.fn((id: string) => Promise.resolve(details(id === finalPeer.id ? finalPeer : item(id)))),
    });
