...
export type ConnectionLookup =
  { kind: 'none' } | { kind: 'match'; connection: PostgresConnection } | { kind: 'conflict' };
...
function parseAccess(value: unknown, authority: ConnectionAuthority): AccessRequest {
  ...
  if (value.scope === 'database') {
    if (
      ...
      authority.access !== 'database' ||
      value.database !== authority.database
    ) {
      throw invalidConnection();
    }
    return { scope: 'database', operation: value.operation, database: value.database };
  }
  if (typeof value.superuser !== 'boolean' || authority.access !== 'full')
    throw invalidConnection();
...
export async function readPostgresConnection(
  ...
  if (
    !validSummary(connection) ||
    connection.id !== item.id ||
    connection.manager !== item.manager ||
    connection.resource !== item.resource ||
    !isRecord(metadata)
  )
    throw invalidConnection();
  ...
  if (
    ...
    !USERNAME.test(metadata.username) ||
...
function callerLabels(item: RPC.ConnectionItem): Record<string, string> {
  return Object.fromEntries(
    Object.entries(item.labels ?? {}).filter(([key]) => !key.startsWith('postgres.')),
  );
}
...
export function findCompatibleConnection(
  ...
  const matches = connections.filter(
    (connection) =>
      connection.managerId === input.managerId &&
      connection.resourceId === input.resourceId &&
      sameAccess(connection.access, input.access) &&
      sameLabels(callerLabels(connection.item), input.labels),
  );
