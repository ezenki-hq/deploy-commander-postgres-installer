...
describe('loadInstallationProjection', () => {
  ...
  it('loads every page before selecting the stable postgres resource', async () => {
    const otherResources = Array.from({ length: 49 }, (_, index) =>
      resource(`other-${index}`, { type: 'other', name: `other-${index}` }),
    );
    const caller = fakeCaller({
      getMyResources: vi
        .fn()
        ...
        .mockResolvedValueOnce(
          resourcePage([resource('last-other', { type: 'other', name: 'last-other' })], 50, 51),
        ),
    });
  ...
  it('reports a concrete conflict for two stable resources', async () => {
    const caller = fakeCaller({
      getMyResources: vi
        .fn()
        .mockResolvedValue(resourcePage([postgresResource, resource('resource-2')], 0, 2)),
    });
