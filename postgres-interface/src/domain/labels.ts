...
export type ConnectionAuthority =
  { access: 'database'; database: string; origin: DatabaseOrigin } | { access: 'full' };
...
export function parseConnectionLabels(value: unknown): ConnectionAuthority {
  ...
  if (labels['postgres.access'] === 'full') {
    if (
      labels['postgres.database'] !== undefined ||
      labels['postgres.database-origin'] !== undefined
    ) {
...
export function deletionEffect(target: DeletionTarget, peers: DeletionTarget[]): DeletionEffect {
  if (target.access !== 'database' || target.origin !== 'managed') return 'role-only';
  const hasPeer = peers.some(
    (peer) =>
      peer.id !== target.id && peer.access === 'database' && peer.database === target.database,
  );
