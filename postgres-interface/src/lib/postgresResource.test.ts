...
describe('postgres resource contract', () => {
  it('lists every exact non-external postgres resource with exhaustive paging', async () => {
    ...
    await expect(listPostgresResources(caller({ getMyResources }))).resolves.toEqual([
      resource,
      match2,
    ]);
  ...
  it('rejects malformed pagination', async () => {
    const getMyResources = vi
      .fn()
      .mockResolvedValue({ items: [], limit: 50, offset: 10, total: 0 });
    await expect(listPostgresResources(caller({ getMyResources }))).rejects.toBeInstanceOf(
  ...
  it.each([
    ...
    [
      'administrator',
      {
        config: {
          ...details.config,
          metadata: {
            ...details.config.metadata,
            administrator: { username: 'bad', password: '' },
          },
        },
      },
    ],
    ...
    [
      'metadata',
      { config: { ...details.config, metadata: { engine: 'postgres', version: '15' } } },
    ],
    [
      'platform',
      {
    ...
    await expect(
      readPostgresInstallation(
        caller({ getResource: vi.fn().mockResolvedValue({ ...details, ...change }) }),
        resource,
      ),
