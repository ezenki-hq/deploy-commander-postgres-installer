...
const run = (
  action: RPC.RunItem['action'],
  status: RPC.RunItem['status'],
  note = 'note',
): RPC.RunItem => ({
  id: `${action}-${status}`,
  action,
...
): LifecycleActionDeps & { mocks: Record<string, ReturnType<typeof vi.fn>> } {
  const mocks = {
...
describe('database-free lifecycle actions', () => {
  it('starts installation only when latest-run state and resources are absent', async () => {
    ...
    expect(d.mocks.start.mock.calls[0]).toEqual([
      'create',
  ...
  it('does not start installation when a resource contradicts retryable run state', async () => {
    const d = deps({
      getRuns: vi
        .fn()
        .mockResolvedValue({ items: [run('create', 3)], limit: 1, offset: 0, total: 1 }),
      getMyResources: vi
        .fn()
  ...
  it('starts teardown without reading administrator metadata', async () => {
    ...
    expect(d.mocks.start).toHaveBeenCalledWith(
      'teardown',
      'ezenki/deploy-commander-runner:latest',
  ...
  it('rejects active runs as busy', async () => {
    const d = deps({
      getRuns: vi
        .fn()
        .mockResolvedValue({ items: [run('create', 1)], limit: 1, offset: 0, total: 1 }),
    });
  ...
  it('retries installation after a failed run with a fixed non-secret message', async () => {
    const d = deps({
      ...
      waitForRun: vi
        .fn()
        .mockRejectedValue(Object.assign(new Error('runner password output'), { status: 3 })),
    });
  ...
  it('retries teardown after a failed run with a fixed non-secret message', async () => {
    const d = deps({
      getRuns: vi
        .fn()
        .mockResolvedValue({ items: [run('teardown', 3)], limit: 1, offset: 0, total: 1 }),
      getMyResources: vi
        .fn()
        .mockResolvedValue({ items: [resource], limit: 50, offset: 0, total: 1 }),
      waitForRun: vi
        .fn()
        .mockRejectedValue(Object.assign(new Error('secret teardown output'), { status: 3 })),
    });
  ...
  it('preserves an abort during initial state reads', async () => {
    ...
    const d = deps({
      getRuns: vi.fn().mockImplementation(async () => {
        controller.abort();
  ...
  it('monitors the exactly correlated run when start transport fails', async () => {
    ...
    const d = deps({
      start: vi
        .fn()
        .mockImplementation(
          async (_action: string, _runner: string, _metadata: unknown, suppliedNote: string) => {
            note = suppliedNote;
          ...
          async (
            ...
            items:
              limit === 1
                ? []
                : offset === 0
                  ? [{ ...run('create', 0, note), id: 'correlated-run' }]
                  : [],
            limit,
            offset,
            total: limit === 1 ? 0 : 1,
    ...
    await installPostgres(d);
    expect(d.waitForRun as unknown as ReturnType<typeof vi.fn>).toHaveBeenCalledWith(
      d.caller,
      d.events,
      'correlated-run',
      expect.anything(),
    ...
    await expect(
      Promise.resolve((d.caller.start as unknown as ReturnType<typeof vi.fn>).mock.calls[0][3]),
    ).resolves.not.toMatch(/secret/);
  });
          ...
          async (
            ...
            items:
              limit === 1
                ...
                : offset === 0
                  ? [
                      { ...run('create', 0, note), id: 'run-a' },
                      { ...run('create', 0, note), id: 'run-b' },
                    ]
                  : [],
            limit,
            offset,
